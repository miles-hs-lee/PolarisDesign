...
| **v0.8.0-rc.2 (현재)** | **Codex 후속 리뷰 5건 fix** — codemod-v08의 `brand.secondary` import 깨짐 fix (실 사용 namespace 감지 후 import 자동 normalize, `ai.*` / `accentBrand.*` 누락 시 첫 polaris import에 자동 추가, dot 뒤 identifier 필수로 prose 오탐 방지), agent-facing docs v0.8 일괄 정렬 (root/template `AGENTS.md` + PostToolUse 훅 + `polaris-component`/`polaris-check` slash command), `no-deprecated-polaris-token` 룰을 typography/radius/ramp 제거 alias까지 확장 (codemod와 동일 set 커버), DatePicker `required` jsdoc 사실관계 정정 (hidden input은 validation/payload 효과 없음 — schema validation 권장), 공개 코드 샘플의 `hint=`/`variant="outline"` 잔존 fix (packages/ui/README · docs/recipes · docs/nextjs-app-router) |
