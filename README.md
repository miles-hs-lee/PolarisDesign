...
| **v0.8.0-rc.6 (현재)** | **Codex P1 2건 + P2 1건 fix** — codemod의 `<HStack>`/`<VStack>` → `<Stack>` rewrite 후 import dedupe 누락 (`import { Stack, Stack, Stack }` 빌드 즉사) + multi-line import body corruption (`{ ..., Stack, state }` 처럼 한 줄 jam) 두 결함 fix. 새 `dedupePolarisImports` post-pass + `parseImportBody`/`rebuildImportBody` 헬퍼로 multi-line 보존 정확히. 추가로 `<TableHead nowrap>` prop 추가 (v0.7.7에서 `<TableCell>`에만 추가됐던 API 비대칭 정리, sortable 모드 호환). codemod 27→29, ui tests +2 |
