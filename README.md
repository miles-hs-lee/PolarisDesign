...
| **v0.8.0-rc.5 (현재)** | **Codex P1 1건 fix — codemod idempotency 안전망** — `normalizePolarisImports`가 마이그레이션 대상이 아닌 파일에도 import를 추가하던 false-positive (예: `const line = { normal: 1 }`만 있는 파일에 `import { line }` 주입해서 duplicate-declaration). 두 단계 안전망: (1) `transform()`에서 이전 rewrite가 변경을 만들었을 때만 post-pass 실행 (rewrite gate), (2) 그래도 통과한 파일에서 `const|let|var|function|class <ns>` 로컬 선언과 충돌하는 namespace는 import 추가 skip. 27 codemod tests (+2 안전망 회귀) |
