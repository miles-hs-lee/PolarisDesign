...
| **v0.7.7 (현재)** | **컴포넌트 완성도 리뷰 + 코덱스 리뷰 9건 — 신규 7종 (`Combobox` · `PageHeader` · `SectionHeader` · `Accordion` · `CircularProgress` · `AvatarGroup` · `NavbarItem`) + Tabs underline variant + 13개 컴포넌트 props 추가 (autoResize / prefix-suffix / clearable / dismissible / iconLeft+Right / interactive 등). a11y/state-sync fix 9건. Demo 카탈로그 51 → 45 섹션 통합 + 6 카테고리 탭. (51 → 58 family · 235 tests)** |
| v0.7.x | 컨슈머/코덱스 후속 리뷰에 따른 patch |
| v0.8.0 | **BREAKING 청소** — deprecated alias 제거 (Tailwind/TS 토큰, Button outline, radius.full) + `Input.hint` → `helperText` rename + `HStack`/`VStack` 제거 (Stack 단일화). 컨슈머 측 `polaris-codemod-v08` 1회 실행으로 자동 마이그레이션 |
