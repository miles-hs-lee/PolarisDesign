...
- `/#/components` — Tier 0 + Tier 1 + Tier 2 컴포넌트 25개 카탈로그
...
- **컴포넌트 25개** — Radix UI 위에 폴라리스 토큰으로 스타일링.
...
- **v0.2.0 (현재)** — Tier 2 컴포넌트 7개(Checkbox/Switch/Skeleton/Alert/Pagination/Breadcrumb/EmptyState), Toast `useToast`/`<Toaster>` imperative API, Card `asChild`, lint false-positive 보정 (layout 화이트리스트 + `<button type="submit">` 허용), SSR-safe theme toggle, status hover/on-status 색상 토큰, Tailwind v4 임시 가이드 (`docs/tailwind-v4-migration.md`)
- **v0.2.x** — 사내 npm 레지스트리 publish, tokens.md hex 사인오프, 첫 파일럿 위반율 측정 baseline
- **v0.3** — Tailwind v4 네이티브 preset, 시각 회귀 테스트, Pretendard 로컬 호스팅, Table 등 추가 예시 화면
