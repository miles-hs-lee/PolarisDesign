...
| **v0.8.0-rc.4 (현재)** | **Codex 후속 리뷰 P1 1건 fix + P2/P3 정리 + e2e baseline 사인오프 흡수** — codemod-v08 normalizePolarisImports의 검사 namespace가 `brand`-family만 보던 누락 fix (surface.raised → layer.surface rewrite 후 layer 자동 import). `background.normal/alternative` / `radius.full` TS member-access rewrite 추가. lint 룰 (`no-arbitrary-tailwind` / `no-hardcoded-color`) 메시지의 v0.8 제거 alias 예시 (`bg-brand-primary` 등) → spec 이름으로. `docs/tailwind-v4-migration.md` v4 theme snippet도 v0.8 토큰. e2e baseline 30/30 ✓ — 정식 v0.8.0 태그 게이트 모두 closed |
