...
| **v0.8.0-rc.3 (현재)** | **Codex 추가 후속 3건 fix** — codemod-v08의 `import type`에 값 namespace 추가하던 버그 fix (consumer typecheck 깨짐 — type-only / value import 분리, value import 없으면 새 라인 합성), 잔여 "alias 아직 작동" 안내 정리 (`AGENTS.md` typography/file-icon alias 안내 / `template-next/DESIGN.md` 토큰 이름 / `apps/demo/Tokens.tsx` description / `tailwind-v4-migration.md` v3↔v4 호환 안내), e2e visual baseline 12건 enumerate (의도된 demo content 추가 + v0.8 토큰 미세 shift — 사용자 사인오프 후 baseline 갱신 별도 PR) |
