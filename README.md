...
| v0.7.3 | 🐛 hotfix — `accent-brand-normal-subtle` Tailwind alias 누락으로 12+ 컴포넌트 hover/active 배경이 silently broken이던 버그 정정 |
| **v0.7.3 (현재)** | **디자인팀 v0.7.3 재검수 + 외부 사이트 검수 결과 정합 — `polaris-helper` 토큰 / FormMessage 자동 ErrorIcon / 신규 lint 룰 3종 / `/proposal-platform` 데모 / `/polaris-brand-audit` / `pnpm verify`** |
