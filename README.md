...
- `/#/tokens` — 디자인 토큰 자동 카탈로그 (시맨틱·램프·서브 팔레트·Spacing·Radius·Shadow·Motion·Z-index·Breakpoint) — `@polaris/ui/tokens`에서 auto-iterate
- **`/#/icons`** — 아이콘 카탈로그 (65 UI 아이콘 × 3 사이즈 + 29 파일 아이콘 + 4 로고)
- `/#/assets` — 디자인 자산 자료실 (figma-spec PNG 인라인)

상단 Navbar의 사용자 Avatar → DropdownMenu에서 라이트/다크 모드를 즉시 전환할 수 있습니다.

## v0.7 highlights

- **DESIGN.md 정의서 완전 정렬** — 디자인팀의 v1 (2026.05) spec에 맞춰 토큰 명명·값 재정렬. 시맨틱 토큰 19개 추가 (`label.*` / `background.*` / `layer.*` / `interaction.*` / `fill.*` / `line.*` / `accentBrand.*` / `accentAction.*` / `focus.ring` / `staticColors.*` / `state.*`).
- **컬러 primitive 확장** — 10단계 (`05`/`90` 추가) × 11 family (브랜드 6 + Sky Blue/Blue/Violet/Cyan/Yellow/Gray).
- **Typography H1-H5 + Heading4 18px + Body3 13px + Caption2 11px** — Caption weight 700, body letter-spacing 제거, 모바일 자동 축소.
- **새 토큰 시스템 4개** — Spacing (12 named) · Z-index (6) · Motion (duration + easing) · Breakpoint.
- **디자인팀 자산 통합** — `@polaris/ui/icons` (65종 × 18/24/32 px) · `@polaris/ui/file-icons` (29 파일) · `@polaris/ui/logos` (Polaris + Nova).
- **자동 코드모드** — `pnpm dlx @polaris/lint polaris-codemod-v07 --apply src` 로 v0.6 / rc.0 → v0.7 일괄 변환.

마이그레이션 가이드: [`docs/migration/v0.6-to-v0.7.md`](docs/migration/v0.6-to-v0.7.md).
...
- **Codex / Cursor / 기타** — 루트 [`AGENTS.md`](AGENTS.md)에 같은 절차를 텍스트 가이드로 제공. 작업 종료 전 `pnpm lint`(또는 `pnpm --filter <pkg> lint`)를 직접 실행하도록 명시. 템플릿에서 시작한 새 앱은 [`packages/template-next/AGENTS.md`](packages/template-next/AGENTS.md)를 함께 가져감.
...
cd PolarisDesign
pnpm install                              # @polaris/ui prepare hook이 자동으로 토큰/아이콘 source 생성
pnpm --filter polaris-template-next dev   # template의 prep:ui가 @polaris/ui dist 빌드
# → http://localhost:3000 — 그대로 본인 앱으로 발전시키거나 packages/ 또는 apps/ 아래 새 디렉터리로 복사
```

데모를 실행하려면:
```sh
pnpm --filter demo dev   # → http://localhost:5173 — 컴포넌트/토큰/아이콘 카탈로그
```
