...
- `/#/components` — Tier 0 + Tier 1 + Tier 2 + Tier 2.5 컴포넌트 30개 카탈로그
...
- **컴포넌트 30개** — Radix UI 위에 폴라리스 토큰으로 스타일링.
...
- **v0.3.0 (현재)** — Tier 2.5 컴포넌트 5개(Stack/Container/Drawer/Table/DescriptionList), DropdownMenuFormItem (server action race condition), Pagination/Card/Checkbox/EmptyState API 보강, 5건 docs (variant-axes·patterns·nextjs·migration·recipes)
- **v0.3.x** — 사내 npm 레지스트리 publish, tokens.md hex 사인오프, 파일럿 위반율 측정 baseline
- **v0.4** — Form/FormField (RHF + zod), Combobox, DataTable (sortable·virtualized), Tailwind v4 네이티브 preset, 시각 회귀 테스트
