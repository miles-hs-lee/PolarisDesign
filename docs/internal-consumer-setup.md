...
> ⚠️ **`^0.7.2` 같은 semver range는 동작하지 않습니다.** URL이 정확히 박힙니다. 새 버전을 받으려면 URL의 두 군데(`v0.7.3` / `0.7.2`)를 직접 갱신하세요. → 자동화는 [Renovate](#renovate-자동-업그레이드-옵션) 섹션 참고.
...
curl -L -o /tmp/SHA256SUMS.txt \
  https://github.com/PolarisOffice/PolarisDesign/releases/download/v0.7.3/SHA256SUMS.txt
